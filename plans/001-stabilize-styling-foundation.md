...
2. **Step 2 turns on an animation that has never run.** `animate-slide-down` is also
...
>
> The same cascade rule has a live consequence worth knowing: at `src/App.tsx:564-568`
> the **pause button** carries `.btn-tool` *and* `bg-red-500`/`bg-green-500`/`text-white`.
> `.btn-tool` wins, so **the pause button never shows its red/green state today.**
> That is a real bug. Do **not** fix it here — it is fixed by the plan 004 toolbar
> migration, where a CVA variant puts those colours in the same layer and the state
> starts working. Plan 004 is told to expect it.
