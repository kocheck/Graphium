    ...
    >
      ...
      >
          ...
          {drawings.map((line) =>
            line.pressures && line.pressures.length > 0 ? (
              ...
              <Line
                ...
                onDragEnd={(e) => updateDrawingTransform(line.id, e.target.x(), e.target.y(), 1)}
              />
            ),
          )}
          {tempLine && (
        ...
        <Layer ref={tokenLayerRef}>
          ...
          {/* Snap Preview */}
          {isDraggingToken &&
            Array.from(snapPreviewPositionsRef.current.entries()).map(([tid, pos]) => {
              const t = resolvedTokens.find((r) => r.id === tid);
              if (!t) return null;
              const geo = createGridGeometry(gridType);
              const cell = geo.pixelToGrid(
                pos.x + (gridSize * t.scale) / 2,
                pos.y + (gridSize * t.scale) / 2,
                gridSize,
              );
        ...
        <Layer>
          <Transformer
            ...
            onTransformEnd={(e) => {
              ...
              // Update the token's transform with the new position and scale
              if (selectedIds.length === 1) {
                updateTokenTransform(selectedIds[0], node.x(), node.y(), scale);
              }

              // Reset the node's scale to 1 since we've stored the scale value
              node.scaleX(1);
              node.scaleY(1);
            }}
          />
      ...
      {isCalibrating && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-black/80 text-white px-4 py-2 rounded-full text-sm font-medium z-50 flex items-center gap-3">
          <span>
            Draw a box around ONE {gridType.startsWith('HEX') ? 'Hex' : 'Square'} to calibrate
          </span>
          <button
