...
class PrivacyErrorBoundary extends Component<Props, State> {
  ...
  static getDerivedStateFromError(): Partial<State> {
    return { hasError: true, sanitizing: true };
  }

  componentDidCatch(error: Error, errorInfo: ErrorInfo): void {
    // Trigger async sanitization without blocking the lifecycle
    this.sanitizeError(error, errorInfo);
  }

  private async sanitizeError(error: Error, errorInfo: ErrorInfo): Promise<void> {
    try {
...
================================================================================
  ...
  render(): ReactNode {
    const { hasError, sanitizing, sanitizedError, copyStatus, saveStatus, userContext, showContextInput } = this.state;
        ...
        <div className="min-h-screen bg-neutral-900 text-white flex items-center justify-center p-8">
          <div className="max-w-2xl w-full bg-neutral-800 rounded-lg shadow-xl overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              {sanitizing ? (
                // Loading state while sanitization is in progress
                <div className="flex flex-col items-center justify-center py-8 space-y-4">
                  ...
                  <p className="text-sm text-neutral-400 text-center">
                    Sanitizing error details to protect your privacy
                  </p>
                </div>
              ) : (
                <>
